...
model_info <- load_model_info(here::here("data/model_info.yaml"))
tasks <- load_eval_results(here::here("results_rds"))

are_eval <-
  process_eval_data(tasks, model_info) |>
...
are_costs <-
  compute_cost_data(tasks, model_info) |>
  filter(model_join %in% INCLUDED_MODELS)
...
<!--
:::{.callout-tip}
If you're interested in Python code generation, we also evaluated how well various models perform on [Pandas code generation](https://posit.co/blog/python-llm-evaluation/). 
:::
-->
