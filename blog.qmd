...
are_eval |> 
  ...
  labs(
    ...
    fill = "Score",
    subtitle = 
      "OpenAI's newest model, GPT-5, performed the best, with o4-mini and o3 close behind."
  ) +
  theme_light() +
  theme(
...
are_eval_summary |> 
  ...
  coord_cartesian(xlim = c(0, 8), ylim = c(0.4, 0.67)) +
  labs(
    x = "Total cost",
    y = "Percent correct",
    title = "Model performance on R code generation vs. cost",
    subtitle = 
      "Cost reflects the actual total charges incurred. gpt-oss-20b and gpt-oss-120b are technically\nfree to run, though you may need to pay to host them."
  ) +
  theme_light() + 
...
model_prices |> 
  ...
  fmt_number(columns = c(`Input tokens used`, `Output tokens used`), decimals = 0) |> 
  tab_header(
